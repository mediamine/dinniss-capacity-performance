...
-- independent views which are only copies of an existing table
drop view if exists time CASCADE;
CREATE OR REPLACE VIEW time AS
SELECT *
FROM synchub_workflowmax2_13307_1_time;

drop view if exists jobassignee CASCADE;
CREATE OR REPLACE VIEW jobassignee AS
SELECT *
FROM synchub_workflowmax2_13307_1_jobassignee;

drop view if exists jobtask CASCADE;
CREATE OR REPLACE VIEW jobtask AS
SELECT *
FROM synchub_workflowmax2_13307_1_jobtask;

drop view if exists jobdetails CASCADE;
CREATE OR REPLACE VIEW jobdetails AS
SELECT *
FROM synchub_workflowmax2_13307_1_jobdetails;

drop view if exists jobtaskassignee CASCADE;
CREATE OR REPLACE VIEW jobtaskassignee AS
SELECT *
FROM synchub_workflowmax2_13307_1_jobtaskassignee;

drop view if exists clientdetails CASCADE;
CREATE OR REPLACE VIEW clientdetails AS
SELECT
    ...
    "BankAccountName",
    "BankAccountNumber"
FROM synchub_workflowmax2_13307_1_clientdetails;

drop view if exists task CASCADE;
CREATE OR REPLACE VIEW task AS
SELECT *
FROM synchub_workflowmax2_13307_1_task;
...
SELECT 
    ...
    EXISTS (
        SELECT 1 
        FROM excel_public_holidays eph 
        WHERE eph.Date = gs::date
    ) AS "PublicHoliday",
    EXTRACT(DOW FROM gs)::int + 1 AS "Weekday",
    (EXTRACT(DOW FROM gs)::int + 1) < 2 OR (EXTRACT(DOW FROM gs)::int + 1) > 6 AS "WeekEnd",
    DATE_TRUNC('month', gs)::date AS "StartOfMonth",
    (DATE_TRUNC('month', gs) + INTERVAL '1 month' - INTERVAL '1 day')::date AS "EndOfMonth",
    (
...
SELECT 
    metric AS "Metric",
    'Selected Metric: ' || SUBSTRING(metric FROM 4) AS "Name",
    CASE metric
...
SELECT
    ...
    jd."CompletedDate"
FROM jobdetails jd
LEFT JOIN clientdetails cd
    ON cd."UUID" = jd."ClientUUID"::uuid
LEFT JOIN jobassignee ja
    ON ja."JobID" = jd."RemoteID";
...
SELECT
    (jt."JobDetailsRemoteID" || jt."UUID" || jta."UUID") AS "Job_Task_Staff_ID",
...
FROM jobtask jt
LEFT JOIN jobtaskassignee jta
    ON jta."JobTaskID" = jt."RemoteID"::uuid
LEFT JOIN jobdetails jd
    ON jd."RemoteID" = jt."JobDetailsRemoteID"
LEFT JOIN clientdetails cd
    ON cd."UUID" = jd."ClientUUID"
...
FROM time t
LEFT JOIN "key03_staff_table" s
    ON s."Staff_UUID" = t."StaffMemberUUID"
...
SELECT DISTINCT
    ...
    COALESCE(
        (
            ...
            WHERE jt."Task_Name" ILIKE ('%' || kt."Name" || '%')
            LIMIT 1
        ),
        jt."Task_Name"
    ) AS "Task_Type1"
...
LEFT JOIN "1_Job_Task_Details_Table" jt
    ON jt."Job_Task_Staff_ID" = combined."Job_Task_Staff_ID"
LEFT JOIN "key06_job_table" j
    ON j."Job_ID" = combined."Job_ID";
