...
class SQLScriptRunner:
    ...
    def run_sql_script(self, sql_content: str, script_name: str = "inline", 
        ...
        try:
            self.conn.commit()
            logger.info(f"SQL script {script_name} completed: {executed} successful, {failed} failed")
            return success
        except psycopg2.Error as e:
    ...
    def run_sql_directory(self, directory_path: str, 
                         pattern: str = "*.sql",
                         continue_on_error: bool = False) -> bool:
        """
    ...
    def create_view_from_query(self, view_name: str, query: str, 
                               replace: bool = True, materialized: bool = False) -> bool:
        """
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                logger.info(f"Creating {'materialized ' if materialized else ''}view: {view_name}")
                cursor.execute(create_sql)
                self.conn.commit()
                logger.info(f"Successfully created {'materialized ' if materialized else ''}view: {view_name}")
                return True
    ...
    def refresh_materialized_view(self, view_name: str, concurrently: bool = False) -> bool:
        """
...
def run_post_sync_sql(connection_string: str, 
                     sql_files: Optional[List[str]] = None,
                     sql_directory: Optional[str] = None,
                     refresh_materialized_views: Optional[List[str]] = None) -> bool:
    """
